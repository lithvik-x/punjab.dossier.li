// Local Body Urban Politics Data
// Derived from: b26-local-body-urban-politics.md

import type {
  MunicipalCorporation,
  UrbanVoterCharacteristic,
  CityPoliticalDynamics,
  MCOutcomeSignal,
  MayoralRole,
  PartyUrbanStrategy,
  AssemblySeatClassification,
} from "@/types/local-body-urban-types";
...
/**
 * Get municipal corporation by city name
 */
export function getMunicipalCorporation(city: string): MunicipalCorporation | undefined {
  return MAJOR_MUNICIPAL_CORPORATIONS.find(m => m.city === city);
}

/**
 * Get city political dynamics by city name
 */
export function getCityDynamics(city: string): CityPoliticalDynamics | undefined {
  return CITY_POLITICAL_DYNAMICS.find(c => c.city === city);
}

/**
 * Get MC outcome signal by result type
 */
export function getMCOutcomeSignal(mcResult: string): MCOutcomeSignal | undefined {
  return MC_OUTCOME_SIGNALS.find(s => s.mcResult.includes(mcResult));
}

/**
 * Get party urban strategy by party name
 */
export function getPartyUrbanStrategy(party: string): PartyUrbanStrategy | undefined {
  return PARTY_URBAN_STRATEGIES.find(p => p.party === party);
}

/**
 * Get assembly seat classification by type
 */
export function getAssemblySeatClassification(type: string): AssemblySeatClassification | undefined {
  return ASSEMBLY_SEAT_CLASSIFICATIONS.find(s => s.type.includes(type));
}

/**
 * Get total urban seats (urban + semi-urban)
 */
export function getTotalUrbanAndSemiUrbanSeats(): number {
  return ASSEMBLY_SEAT_CLASSIFICATIONS.reduce((sum, c) => sum + c.approximateSeats, 0);
}
